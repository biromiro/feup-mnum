...
#def bissection_absolute_precision(a,b,f):

#def bissection_relative_precision(a,b,f):

#def bissection_function_annulment(a,b,f):

#def bissection_max_precision(a,b,f):
...
def euler_improved(x,y,z,xf,n):
    ...
    for i in range(3):
        for j in range(n + 1):
            pny1 = y + 2*f1(x1,y1,z1)*h
            pnz1 = z + 2*f2(x1,y1,z1)*h
            pndy1 = f1(x1,pny1,pnz1)
            pndz1 = f2(x1,pny1,pnz1)
            x = x1
            y = y1
            z = z1
            x1 = x1 + h
            y1 = y1 + (pndy1 + f1(x1,y1,z1))*h/2
            z1 = z1 + (pndz1 + f1(x1,y1,z1))*h/2
            if (i == 0):
                t.append(x)
                mi.append(y)
                mp.append(z)
